...
class SlackWatcher {
  ...
  updateConfig(config) {
    ...
    if (config.channels && Array.isArray(config.channels)) {
      ...
      console.log('✅ SlackWatcher: 復元完了:', Array.from(this.watchedChannels));
    }
  }
  ...
  async connect() {
    ...
    try {
      ...
      this.socketClient.on('message', (message) => {
        console.log('📩 Socket Mode全メッセージ:', JSON.stringify(message, null, 2));
      });
  ...
  async addWatchChannel(channelId) {
    ...
    try {
      ...
      if (!channelInfo.is_member) {
        console.warn('⚠️ ボットがチャンネルに参加していません:', channelInfo.name);
        console.warn('💡 チャンネルにボットを招待してください: /invite @' + (await this.webClient.auth.test()).user);
      }
    } catch (error) {
  ...
  async processMessage(slackEvent, originalEvent) {
    ...
    try {
      ...
      // チャンネル情報を取得
      const channelInfo = await this.getChannelInfo(slackEvent.channel);
      console.log('📋 チャンネル情報:', channelInfo);
